...
from dataset.mnist import get_mnist_dataloaders, get_mnist_dataloaders_sample
...
def parse_option():
    ...
    # dataset
    parser.add_argument('--dataset', type=str, default='cifar100', choices=['cifar100', 'mnist'], help='dataset')
    ...
    parser.add_argument('--model_s', type=str, default='MobileNetV2Trofim',
                        choices=['resnet8', 'resnet14', 'resnet20', 'resnet32', 'resnet44', 'resnet56', 'resnet110',
                                 ...
                                 'MobileNetV2', 'MobileNetV2Trofim', 'ShuffleV1', 'ShuffleV2', 'fc5'])
...
def main():
    ...
    if opt.dataset == 'cifar100':
        ...
        if opt.distill in ['crd']:
            train_loader, val_loader, test_loader, n_data = get_mnist_dataloaders_sample(batch_size=opt.batch_size,
                                                                               num_workers=opt.num_workers,
                                                                               k=opt.nce_k,
                                                                               mode=opt.mode, part = opt.part)
        else:
            train_loader, val_loader, test_loader, n_data = get_mnist_dataloaders(batch_size=opt.batch_size,
                                                                        num_workers=opt.num_workers,
                                                                        is_instance=True, part = opt.part)
        n_cls = 10
    ...
    if opt.model_s == 'fc5':
        ...
        with open(opt.arcs_dir + '/fc5.json', 'r') as infile:
            arcs = infile.read()

        arcs = eval(arcs)
        model_t = DenseModel(arcs[0])
    ...
    elif opt.model_s == 'fc5':

        with open(opt.arcs_dir + '/fc5.json', 'r') as infile:
            arcs = infile.read()

        arcs = eval(arcs)
        model_s = DenseModel(arcs[opt.arc])
    ...
    for elem in model_s.parameters():
        print(1, elem)
    ...
    if opt.model_s == 'fc5':
        optimizer = optim.SGD(trainable_list.parameters(), lr=1e-3, momentum=0.9)
    else:
